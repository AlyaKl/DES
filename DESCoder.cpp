...
void DESCoder::_IP()
{
	...
	for (int i = 0; i < 64; i++)
		newDataBits[i] = _dataBits[ ip[i] - 1 ]; // собственно, сама перестановка
...
void DESCoder::_inverseIP()
{
	...
	vecUnsChar newDataBits(64); // куда будет записана перестановка
	for (int i = 0; i < 64; i++)
		newDataBits[i] = _dataBits[ fp[i] - 1 ]; // собственно, сама перестановка
...
//		подстановки в 8 S-блоках
//		перестановки в P-блоке
...
// S-блоки - таблицы 4x16, каждый элемент которой - 4-битное число
...
vecUnsChar DESCoder::_bytesArrayToBits(vecUnsChar bytes)
{
	...
	for (int i = 0; i < bytes.size(); i++)
	{
		...
		vecUnsChar iByteBits(8);
		int j = 0;
		while (bytes[i] > 0)
		{
			iByteBits[j] = bytes[i] % 2;
			j++;
			bytes[i] /= 2;
		}
